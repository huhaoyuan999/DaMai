...
(function () {
    $("img[original]").lazyload({effect: "fadeIn", failurelimit: 0, threshold: 0});
})();
var damai = {
    page: {
        settings: {
            get: function (key, value) {
                ...
                if (typeof itemSettings[key] == "undefined") {
                    return value;
                }
...
(function ($) {
    ...
    String.prototype.template = function (obj) {
        return this.replace(/\{([a-z_]+)\}/gi, function (m, s) {
            return obj[s] || "";
        });
    };
    ...
    function setBtnXuanzuoStatus(isXuanzuo) {
        if (isXuanzuo) {
            $("#btnXuanzuo, #btnXuanzuo3").show();
            $("#btnXuanzuo2").removeAttr("style");
        } else {
    ...
    function removeCartPrice(performId, priceId) {
        if (!priceId) {
            ...
            if (cartMoving["mv_{0}_{1}".format(performId, priceId)]) {
                return;
            }
    ...
    function initYhcx() {
        ...
        $.get("/ajax/yhcx.html", {
            projectid: projectInfo.ProjectID,
            cityid: $("#cityId").val(),
            categoryID: $("#ChildCategoryID").val()
        }, function (data) {
            data = eval("(" + data + ")");
            if (data.Status == 200) {
                var funs = {
                    T1: function (date) {
                        return "每日" + date.substr(11, 5);
                    },
                    T7: function (date) {
                        return "每周" + ["日", "一", "二", "三", "四", "五", "六"][new Date(date.replace(/-/gi, "/")).getDay()] + date.substr(11, 5);
                    },
                    T30: function (date) {
                        return "每月" + date.substr(8, 2) + "日" + date.substr(11, 5);
                    }
                }, typecss = {
                    T1: "zhekou",
                    T2: "manjian",
    ...
    function initGexhTuijian() {
        if (projectInfo.OptimizationTicket == 1) {
            $("#hotProjects").remove();
            return false;
        }
        $.get("/ajax/cainiXihuan.html", {
            type: 1,
        ...
        function parse(str) {
            try {
                ...
                for (var i in t) {
                    ...
                    if ($i.length != 3 || id == projectInfo.ProjectID) {
                        continue;
                    }
                    l.push({
                        i: id,
    ...
    var setmailChannel = window.setmailChannel = function () {
        ...
        $.get("/ajax/setChanelIds.html", {
            account: changemailnum,
    ...
    function loadu(u) {
        ...
        if (u) {
            ...
            (function () {
                ...
                if (($ele.val() || "").length != 0 && $ele.val() != $ele.attr("placeholder")) {
                    return;
                }
                $ele.val(u.phone);
            })();
    ...
    function initPage(project) {
        var performList = $("#performList"), priceList = $("#priceList"), cartList = $("#cartList"),
            buyButtonC = $("#buyButtonC");
        ...
        var isComingSoon = false, isJinpai = false, jinpaiStatus = 0, isPrivilege = false, privilegeStatus = false,
            ...
            function updatePrivilegeStatus(isLogin, itm) {
                ...
                try {
                    ...
                    timer = setInterval(function () {
                        ...
                        var status = conitm.attr("data-status") - 0, type = conitm.attr("data-type") - 0,
                            conditions = conitm.attr("data-conditions");
                        if (status == 200) {
                            //验证通过
        ...
        (function () {
            ...
            var jinpaiMark = $("#jinpaiMark"), jinpaiCounter = $("#jinpaiCounter"), jinpaiTabs = $("#jinpaiTabs"),
                jinpaiOrders = $("#jinpaiOrders"), jinpaiLives = $("#jinpaiLives"),
                jinpaiGroupList = $("#jinpaiGroupList");
            if (jinpai == null) {
                jinpaiMark.remove();
            ...
            var changetime = jinpai.changetime, starttime = jinpai.starttime, endtime = jinpai.endtime,
                groups = jinpai.groups;
        ...
        function updateStatus() {
            ...
            if (!isJinpai && !isComingSoon && privilegeStatus) {
                ...
                //默认选中场次与票价
                (function () {
                    var perc = performList.find("ul.lst li.itm:not(.itm-mobile,.itm-oos)"),
                        pric = priceList.find("ul.lst li.itm:not(.itm-mobile,.itm-oos)");  // itm-sel
                    if (perc.length == 1) {
    ...
    $(document).ready(function () {
        ...
        .on("click", "#getcode_btnl:not(.z-dis)", function () {
            ...
            $.get("/ajax/getcode.html", {
                phonenum: changephonenum,
                token: tokenvalue,
                t: Math.random()
            }, function (data) {
                data = eval('(' + data + ')');
        ...
        .on("click", "#lijidy, #dy3", function () {
            location.href = 'http://my.damai.cn/account/mysubscribe';
        })
        .on("click", "#dy2", function () {
            ...
            !function () {
                ...
                $.getJSON("/ajax/GetSubscribeChannels.html", {
                    projectId: projectInfo.ProjectID,
                    t: Math.random()
                }, function (data) {
                    if (data.Status == 200) {
        ...
        .on("click", "div.m-choose.m-choose-date ul.lst:not(.lst-dis) li.itm:not(.itm-more,.itm-oos)>a", function () {
            var $this = $(this).parent(), performId = $this.attr("data-performId"),
                performtime = new Date($this.attr("data-performTime").replace(/\-/g, '/')).getTime();
        ...
        .on("click", "#btnBuyTicketValidate", function () {
            ...
            $.get("/ajax/buyTicketValidate.html", {
                projectId: projectInfo.ProjectID,
                v: v,
                t: Math.random()
            }, function (rsp) {
